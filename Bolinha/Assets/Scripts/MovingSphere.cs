...
public class MovingSphere : MonoBehaviour {
	...
	[SerializeField]
	Transform playerInputSpace = default, ball = default, cameraFocusPoint = default;
    ...
    bool Grappling => grapplingGun.IsGrappling();
	...
	void FixedUpdate () {
		...
		ClearState();
        cameraFocusPoint.transform.position = transform.position + (upAxis * focusPointHeight);
	}
	...
	void AdjustVelocity () {
		...
		if (Climbing) {
            ...
            acceleration = playerInput == Vector3.zero ? maxGrappleAcceleration : maxAirAcceleration;
            speed = maxGrappleSpeed;
            xAxis = rightAxis;
            zAxis = forwardAxis;
        }
		else {
